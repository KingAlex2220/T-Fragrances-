...
FRAGRANCE_CATALOG = [
    # --- MEN'S COLLECTION (30) ---
    {"id": "m01", "name": "Savage Spirit Blend", "gender": "Men", "category": "Fresh & Spicy", "price": 45.0, "notes": "Inspired by Sauvage profile — Crisp bergamot, pepper, and rich ambroxan.", "image_url": None},
...
# ==========================================
# SIDEBAR NAVIGATION & QR CODE SECTION
# ==========================================
st.sidebar.title("✨ T Fragrances")
st.sidebar.caption("50ml Clear Bottle Luxury Impressions ($45.00)")
...
st.sidebar.markdown("---")
st.sidebar.subheader("📲 Scan to Order")
if os.path.exists("qr_code.png"):
    st.sidebar.image("qr_code.png", caption="Scan to open store", use_container_width=True)
else:
    st.sidebar.info("Place your QR image file as 'qr_code.png' in your repository to display it here.")
...
def get_item_price(i_id):
    if i_id.startswith("custom_"):
        return st.session_state.custom_catalog[i_id]["price"]
    else:
        match = [item["price"] for item in FRAGRANCE_CATALOG if item["id"] == i_id]
        return match[0] if match else 45.0

raw_subtotal = sum(get_item_price(i_id) * qty for i_id, qty in st.session_state.cart.items())
...
# ------------------------------------------
# TAB 1: BROWSE CATALOG & CUSTOM REQUESTS
# ------------------------------------------
with tabs[0]:
    st.caption(f"Showing {len(filtered_catalog)} scents | Uniform Retail Price: $45.00 each")
    
    # --- CUSTOM FRAGRANCE REQUEST SECTION (FOR QR CODE SCANNERS) ---
    with st.container(border=True):
        st.subheader("💬 Request a Custom Fragrance Impression")
        st.write("Don't see your favorite designer scent in our standard catalog? Submit your custom request below and add it directly to your order for **$45.00**.")
        
        with st.form("custom_request_form"):
            col_c1, col_c2 = st.columns(2)
            with col_c1:
                custom_frag_name = st.text_input("Designer Fragrance Name / Inspiration *", placeholder="e.g., Tom Ford Lost Cherry, Creed Silver Mountain Water")
                custom_gender = st.selectbox("Category / Gender", ["Men", "Women", "Unisex"])
            with col_c2:
                custom_notes = st.text_input("Specific Notes or Details (Optional)", placeholder="e.g., Extra concentrated oil blend")
                custom_qty = st.number_input("Quantity", min_value=1, max_value=10, value=1)
                
            submitted_custom = st.form_submit_button("Add Custom Request to Bag ($45.00)")
            if submitted_custom:
                if not custom_frag_name:
                    st.error("Please enter the name of the designer fragrance you want.")
                else:
                    custom_id = f"custom_{abs(hash(custom_frag_name)) % 10000}"
                    st.session_state.custom_catalog[custom_id] = {
                        "name": f"Custom: {custom_frag_name}",
                        "gender": custom_gender,
                        "price": 45.0,
                        "notes": custom_notes if custom_notes else "Custom client-requested impression"
                    }
                    if custom_id in st.session_state.cart:
                        st.session_state.cart[custom_id] += custom_qty
                    else:
                        st.session_state.cart[custom_id] = custom_qty
                    st.success(f"Added custom request '{custom_frag_name}' to your bag!")
                    st.rerun()
    ...
    if not st.session_state.cart:
        ...
        for item_id, qty in st.session_state.cart.items():
            if item_id.startswith("custom_"):
                prod = st.session_state.custom_catalog[item_id]
                name = prod["name"]
                cat = prod["gender"]
                price = prod["price"]
            else:
            ...
            summary_list.append(f"{qty}x {name}")
